...
def normalize(text: str) -> str:
    ...
    text = "".join(
        [c for c in text if unicodedata.category(c) in ["Ll", "Nd", "Zs", "Cc"]]
    )
...
def match_phrase(tokens: Doc, ability: str, model: Language) -> bool:
    ...
    if len(matches_list) > 0:
        logger.info(
            f"Ability {ability} found with PhraseMatcher {len(matches_list)} times"
        )
        return True
    else:
        return False
    ...
    *,
    ...
    for ability in abilities:
        ...
        if ability in tokens_text:
            ...
            doc_norm_text = [
                custom_lemmas.get(token.lemma_, token.lemma_)
                for token in doc_norm
                if token.text.strip()
            ...
            if (
                not ability_doc
                or len(ability_doc) == 0
                or not all([token.has_vector for token in ability_doc])
            ):
                result["WORDS_NOT_FOUND"].append(ability)
                logger.info(
                    f"Skipping ability {ability} because it's empty or doesn't have a vector"
                )
                continue
            ...
            for token in token_groups:
                if not token.text.strip():
                    ...
                    logger.info(
                        f"Tried to evaluate similarity of token without vector: {token}"
                    )
                    continue
                sum = 0
            ...
            if similarities and max_value >= similarity_threshold:
                result["WORDS_FOUND"].append(ability)
                logger.info(
                    f'Matched ability "{ability}" ({ability_doc_text}) with max similarity {max_value} to "{max_key}"'
                )
            else:
                result["WORDS_NOT_FOUND"].append(ability)
                logger.info(
                    f'Didn\'t match ability "{ability}" ({ability_doc_text}) with max similarity {max_value} to "{max_key}"'
                )
...
def create_token_groups(doc: Doc, word_amount: int) -> list[Span]:
    ...
    for i in range(len(doc) - word_amount + 1):
        ...
        if word_amount >= 10:
            logger.info(
                f'Skipping token group "{tokens_text}" permutations because word amount is too large ({word_amount})'
            )
        elif word_amount > 1:
